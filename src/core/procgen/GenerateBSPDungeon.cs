...
public partial class GenerateBSPDungeon : Node
{
	...
	private void GenerateNewDungeon()
	{
		...
		AddCorridors();
		FillInGridMapTiles();
		GetDijkstraMap(new Vector2I(0, 2));
		PlaceDownEnemies();
	}
	...
	private Godot.Collections.Dictionary<Vector2I, int> GetDijkstraMap(Vector2I targetPosition)
	{
		Godot.Collections.Dictionary<Vector2I, int> dijkstraMap = new();
		...
		while (frontier.Any())
		{
			...
			for (var i = 0; i < 4; i++)
			{
				...
				if (neighbourCoordinates.X < 0 || neighbourCoordinates.Y < 0) continue;
				
				int tileType = _dungeonGridMap[neighbourCoordinates.X, neighbourCoordinates.Y];
				
				if (dijkstraMap.ContainsKey(currentNeighbour) && tileType != 0)
				{
					dijkstraMap[currentNeighbour] += 1;
				}
				else if (tileType != 0)
				{
					frontier.Enqueue(currentNeighbour);
					dijkstraMap.Add(currentNeighbour, 0);
				}
			}
		}
	...
	private void PlaceDownPlayer()
	{
		...
		Vector2 playerSpawnCoordinates = CursedMerc.utils.Utils.CalculateCenter(1, 2, 3, 4);
